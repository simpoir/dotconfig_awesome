...
micwidget:set_text("🔊")
micwidget_state = -1
micwidgettimer = timer({ timeout = 15 })
micwidgettimer_timeout = function()
      status = os.execute("amixer sget Capture | grep off")
      if status ~= micwidget_state then
         micwidget_state = status
         if status == 0 then
            micwidget:set_markup("<span fgcolor=\"".. color_red .."\">🔊</span>")
         else
            micwidget:set_markup("<span fgcolor=\"".. color_green .."\">🔊</span>")
         end
      end
   end
ptt_debounce = timer({ timeout = 1 }) -- should be longer than key repeat
ptt_debounce:connect_signal("timeout", function()
   awful.util.spawn("amixer sset Capture nocap")
   micwidgettimer_timeout()
   ptt_debounce:stop()
end)

micwidgettimer:connect_signal("timeout", micwidgettimer_timeout)
micwidgettimer:start()
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ }, "XF86AudioRaiseVolume", function()
       ...
       awful.util.spawn("paplay /usr/share/sounds/ubuntu/stereo/message.ogg")
    end),
    ...
    function()
       ptt_debounce:again()
       awful.util.spawn("amixer sset Capture cap")
       micwidgettimer_timeout()
    end),
    ...
    awful.key({ }, "XF86AudioStop", function()
       awful.util.spawn("dbus-send --print-reply --dest=org.mpris.MediaPlayer2.vlc /org/mpris/MediaPlayer2 org.mpris.MediaPlayer2.Player.Stop")
    end),
...
if 0 == os.execute("xinput list | grep TrackPoint")
then
   -- No touchpad if there is a trackpoint
   awful.util.pread("synclient TouchpadOff=1")
end
awful.util.spawn("compton")
awful.util.spawn_with_shell("pkill nm-applet; sleep 5; nm-applet")
awful.util.pread("xset dpms 300 300 300")
